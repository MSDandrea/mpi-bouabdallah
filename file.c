////
//// Created by matheus on 20/06/19.
////
//
//
//
...
//int has_all_res() {
//    for (int i = 0; i < n_rec; ++i) {
//        if (needed_token[i] && !my_tokens[i])
//            return 0;
//    }
//    return 1;
//}
//
//void request_resources(int *requested) {
//    printf("Nó %d quer usar os recursos:", self);
//    for (int i = 0; i < 3; ++i) {
//        printf(" %d", requested[i]);
//        needed_token[requested[i]] = 1;
//    }
//    printf("\n");
//    if (!has_all_res(requested)) { //se ele não possui todos os recursos
//        send_request(self, parent); //envia request
//        pthread_mutex_lock(&access_mutex); //espera pra entrar
//    }
...
//
//
//void send_inquire(int resources[3], int node) {
//
//}
//
//void receive_ack_1(int tokens[3]){
//    for (int i = 0; i < 3; ++i) {
//        if (tokens[i] != -1){
//            my_tokens[tokens[i]] = 1;
//        }
//    }
//}
//
//void receive_token(int *control_token) {
//    for (int i = 0; i < n_rec; ++i) {
//        if (needed_token[i] && control_token[i] == -1) {//pegue todos os tokens que precisa e não tem
//            control_token[i] = self;
//            my_tokens[i] = 1;
//        } else if (control_token[i] == self && !needed_token[i]) {//devolva todos os que tem e não precisa
//            my_tokens[i] = 0;
//            control_token[i] = -1;
//        }
//    }
//    if (!has_all_res()) {
//        for (int i = 0; i < n_procs; ++i) {
//            int resources[3] = {-1, -1, -1};
//            int count = 0;
//            for (int j = 0; j < n_rec; ++j) {
//                if (control_token[j] == i && needed_token[j]) {
//                    resources[count] = j;
//                    count++;
//                }
//            }
//            if (resources[0] != -1) {
//                send_inquire(resources, i);
//            }
//        }
//    } else {
...
//        if (st.MPI_TAG == REQUEST_CONTROL_TOKEN) {
...
//
//void send_request(int origin, int to) {
//    MPI_Send(&origin, 1, MPI_INT, to, REQUEST_CONTROL_TOKEN, MPI_COMM_WORLD);
//}
